...
<body>
  ...
  <div class="container">
  <h1 class="text-center mt-3">Explore Projects </h1>
  <div class="row boxed-portfolio">
  <c:forEach items="${projectData}" var="pd">
      ...
      <div class="custom-job-card h-100 p-4">
        ...
        <div class="job-header d-flex align-items-center gap-3 mb-3">
          <div class="company-logo-circle">
            <img src="./files/serverImgs/${pd.companyProfile}" alt="${pd.companyProfile}" />
          </div>
          <div>
            <span class="company-name d-block">${pd.projectTitle}</span>
            <p>${pd.companyName}</p>
          </div>
        </div>

        <div class="card-body px-0">
          <p class="job-description mb-3" style="min-height: 80px; color: #4b5563;">
            ${pd.projectDescription}
          </p>

          <div class="job-skills my-3">
            <strong style="color: #692A84">Required Skills:</strong>
            <c:if test="${not empty pd.projectSkills}">
              <c:forEach var="skill" items="${fn:split(pd.projectSkills, ',')}">
                <span class="badge skill-badge">${fn:trim(skill)}</span>
              </c:forEach>
            </c:if>
          </div>

          <a href="viewAndApplyProject/${pd.id}" class="btn btn-outline-success w-100 mt-3">
            View and Apply <i class="fa-solid fa-angles-right"></i>
          </a>

          <a href="./files/serverImgs/${pd.projectPdf}" class="btn btn-outline-primary w-100 mt-2" download="ProjectPlus Documentation.PDF">
            Download Documentation PDF &nbsp; <i class="fa-solid fa-file-arrow-down"></i>
          </a>
        </div>
      </div>
    </div>
